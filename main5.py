...
class Ant:
    def __init__(self, X, Y):
        ...
        self.food = 1 if random.uniform(0, 10) > 5 else 0
    ...
    def yellow_cry(self):
        for i in range(count):
            if ants[i].yellow_dis_counter > self.yellow_dis_counter:
                ...
                if self.X - dst < x < self.X + dst and self.Y - dst < y < self.Y + dst:
...
count = 1000
dst = 38
ants = [Ant(random.uniform(50, 1350), random.uniform(50, 750)) for _ in range(count)]
